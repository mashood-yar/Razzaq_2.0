          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-[10px] lg:gap-6">
            {bestSellers.map((product, idx) => (
              <motion.div 
                ...
                transition={{ duration: 0.55, ease: [0.16,1,0.3,1], delay: idx * 0.09 }}
                className={`flex flex-col ${idx % 2 !== 0 ? 'mt-[52px]' : ''} lg:mt-0 group cursor-pointer`}
              >
                <div className="relative aspect-[2/3] w-full bg-[var(--bg-dusk)] rounded-[4px] overflow-hidden mb-3">
                  {product.images?.[0] && (
                    <Image
                  ...
                  <button className="absolute top-2 right-2 w-[36px] h-[36px] bg-[var(--bg-obsidian)]/70 flex items-center justify-center rounded-[2px]">
                    <Heart className="w-4 h-4 text-[var(--rose-dust)]" />
                  </button>
                </div>
                <div className="px-1 flex flex-col flex-1">
                  <h3 className="font-display font-semibold text-[1.2rem] text-[var(--cream-bone)]">{product.name}</h3>
                  ...
                  <div className="flex items-center gap-2 mb-3 mt-auto">
                    <span className="font-body font-bold text-[1.125rem] text-[var(--gold-warm)]">PKR {product.price}</span>
                  </div>
                  <button className="w-full h-[44px] bg-[var(--gold-warm)] text-[var(--bg-void)] font-body font-semibold text-[10px] tracking-[0.2em] rounded-[2px]">
                    ADD TO CART
                  </button>
                </div>
              </motion.div>
