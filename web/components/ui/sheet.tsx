...
const sheetVariants = cva(
  "fixed z-50 flex flex-col gap-4 bg-card p-6 shadow-lg outline-none focus:outline-none data-[state=open]:animate-none data-[state=closed]:animate-none border-border/50",
  {
    variants: {
      side: {
        top: "inset-x-0 top-0 border-b rounded-none rounded-b-xl sm:rounded-b-xl",
        bottom:
...
function sheetSlideInitial(side: "top" | "bottom" | "left" | "right") {
  switch (side) {
    case "right":
      return { x: "100%", y: 0 };
    case "left":
  ...
  <SheetPortal>
    ...
    <SheetPrimitive.Content asChild {...props}>
      <motion.div
        ...
        initial={sheetSlideInitial(side ?? "right")}
        animate={{ x: 0, y: 0 }}
