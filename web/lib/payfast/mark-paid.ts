...
export async function markPayfastOrderPaid(orderId: string, note: string) {
  ...
  if (existing.payment_method !== "payfast") {
  ...
  if (error || !updated) {
    console.error("[PayFast] mark paid:", error);
  ...
  try {
    ...
    console.error("[PayFast] confirmation email:", e);
  }
